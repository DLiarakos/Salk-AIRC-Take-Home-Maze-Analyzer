...
/**
 * Worker-side MP4 demuxing, WebCodecs decoding, background extraction, body tracking, and exact-frame review.
 *
 * Inputs:
 * - Worker requests carrying source video Files and, for tracking, background/calibration/segmentation settings.
 *
 * Outputs:
 * - Video metadata, exact frame timings, representative frames, background models, tracking progress/BodyTrack, and review frames.
 *
 * Main responsibilities:
 * - Demuxes source samples with MP4Box while retaining authoritative PTS/DTS timing.
 * - Decodes frames with WebCodecs for preview/background/tracking passes.
 * - Supports persistent exact-frame review requests without loading the full decoded video into main-thread memory.
 */
...
function debugDecoder(message: string, data?: unknown): void {
  if (!DEBUG_DECODER)
    return;
  if (data !== undefined) {
    console.log(`[Barnes decoder] ${message}`, data);
  }
  else {
    console.log(`[Barnes decoder] ${message}`);
  }
}
...
interface OpenReviewFileRequest {
  type: 'open-review-file';
  file: File;
}

interface DecodeReviewFrameRequest {
  type: 'decode-review-frame';
  requestId: number;
  targetPresentationIndex: number;
  targetSampleIndex: number;
}

type ExtendedDecoderRequest = DecoderRequest | OpenReviewFileRequest | DecodeReviewFrameRequest;
...
type ExtendedDecoderResponse = DecoderResponse | ReviewFileReadyResponse | ReviewFrameResponse | ReviewErrorResponse;
interface CachedReviewSample {
  ...
  */
   ...
   * WebCodecs timestamp.
  */
  decoderTimestampUs: number;
  isKeyFrame: boolean;
  data: Uint8Array;
}
let cancelled = false;
let reviewSamples: CachedReviewSample[] = [];
let reviewDecoderConfig: VideoDecoderConfig | null = null;
let reviewWidth = 0;
let reviewHeight = 0;
let latestReviewRequestId = 0;
let reviewDecodeChain = Promise.resolve();
function post(message: ExtendedDecoderResponse, transfer: Transferable[] = []): void {
...
function waitForDecodeQueue(decoder: VideoDecoder): Promise<void> {
  if (decoder.state === 'closed') {
    return Promise.reject(new Error('VideoDecoder closed unexpectedly.'));
  }
  ...
  return new Promise<void>((resolve, reject) => {
    const check = () => {
      if (decoder.state === 'closed') {
        cleanup();
        reject(new Error('VideoDecoder closed while waiting for decode capacity.'));
        return;
      }
      if (decoder.decodeQueueSize <=
        RESUME_DECODE_QUEUE) {
    ...
    const timer = ctx.setInterval(check, 10);
    const cleanup = () => {
...
function evenlySpacedIndices(frameCount: number, requestedCount: number): number[] {
  if (frameCount <= 0 ||
    requestedCount <= 0) {
    return [];
  }
  ...
  for (let i = 0; i < count; i += 1) {
    indices.add(Math.round((i * lastIndex) /
      (count - 1)));
  }
  return [
    ...indices,
...
async function openReviewFile(file: File): Promise<void> {
  cancelled =
    false;
  reviewSamples =
    [];
  ...
  latestReviewRequestId =
    0;
  const mp4File = (MP4Box as any)
    .createFile(true);
  let videoTrack: Mp4VideoTrack | null = null;
  let nextSampleIndex = 0;
  let extractionComplete = false;
  let demuxReadyResolve!: (track: Mp4VideoTrack) => void;
  let demuxReadyReject!: (reason: unknown) => void;
  const demuxReady = new Promise<Mp4VideoTrack>((resolve, reject) => {
    demuxReadyResolve =
      resolve;
    demuxReadyReject =
      reject;
  ...
  mp4File.onReady =
    async (info: Mp4Info) => {
      try {
        ...
        if (!track) {
          throw new Error('No video track found for exact-frame review.');
        }
        if (!track.codec.startsWith('avc1') &&
          !track.codec.startsWith('avc3')) {
          throw new Error(`Exact-frame review currently supports H.264/AVC MP4 only; found ${track.codec}.`);
        }
        ...
        const support = await VideoDecoder
          .isConfigSupported(config);
        if (!support.supported) {
          throw new Error(`Browser cannot decode exact review frames for ${track.codec}.`);
        }
        videoTrack =
          track;
        reviewDecoderConfig =
        ...
        reviewHeight =
          track.video.height;
        mp4File
          .setExtractionOptions(track.id, null, {
  ...
  mp4File.onSamples =
    (trackId: number, _user: unknown, samples: Mp4Sample[]) => {
      if (!videoTrack) {
        extractionDoneReject(new Error('Exact-review samples arrived before track initialization.'));
        return;
      }
      for (const sample of samples) {
        const pts = asRational(sample.cts, sample.timescale);
        const duration = asRational(sample.duration, sample.timescale);
        const sampleIndex = nextSampleIndex++;
        reviewSamples.push({
          sampleIndex,
          pts,
          duration,
          timestampUs: timeToWebCodecsMicroseconds(pts),
          /*
          ...
          */
          decoderTimestampUs: sampleIndex,
      ...
      const deliveredAll = reviewSamples.length >=
        videoTrack.nb_samples;
      const deliveredFinalSample = last?.number !==
        undefined &&
        last.number + 1 >=
        videoTrack
          .nb_samples;
      ...
      if (last?.number !==
        undefined) {
        mp4File
          .releaseUsedSamples(trackId, last.number + 1);
      }
    };
  /*
  ...
  */
  ...
  if (reviewSamples.length !==
    ...
    throw new Error(`Exact-review cache contains ${reviewSamples.length} samples; expected ${readyTrack.nb_samples}.`);
  }
  post({
    type: 'review-file-ready',
...
async function decodeReviewFrame(request: DecodeReviewFrameRequest): Promise<void> {
  if (!reviewDecoderConfig ||
    reviewSamples.length ===
    0) {
    throw new Error('Exact-frame review file has not finished opening.');
  }
  const target = reviewSamples[request
    .targetSampleIndex];
  if (!target) {
    throw new Error(`Exact review sample ${request.targetSampleIndex} does not exist.`);
  }
  /*
   * Begin decoding at the nearest preceding
   * random-access sample in DECODE order.
  */
  let startSampleIndex = request
    .targetSampleIndex;
  while (startSampleIndex >
    0 &&
    !reviewSamples[startSampleIndex].isKeyFrame) {
    startSampleIndex -=
      1;
  }
  if (!reviewSamples[startSampleIndex].isKeyFrame) {
    throw new Error(`No preceding H.264 key frame found for sample ${request.targetSampleIndex}.`);
  }
  const pendingByDecoderTimestamp = new Map<number, CachedReviewSample>();
  let resolveBitmap!: (bitmap: ImageBitmap) => void;
  let rejectBitmap!: (reason: unknown) => void;
  const bitmapPromise = new Promise<ImageBitmap>((resolve, reject) => {
    resolveBitmap =
      resolve;
    rejectBitmap =
      reject;
  });
  let decoderFailure: unknown = null;
  const decoder = new VideoDecoder({
    output: (frame: VideoFrame) => {
      const source = pendingByDecoderTimestamp.get(frame.timestamp);
      if (source) {
        pendingByDecoderTimestamp.delete(frame.timestamp);
      }
      if (!source) {
        frame.close();
        decoderFailure =
          new Error(`Exact-review decoded timestamp ${frame.timestamp} µs had no matching submitted source sample.`);
        return;
      }
  ...
  decoder.configure(reviewDecoderConfig);
  for (let sampleIndex = startSampleIndex; sampleIndex <=
    request
      .targetSampleIndex; sampleIndex += 1) {
    const sample = reviewSamples[sampleIndex];
    pendingByDecoderTimestamp.set(sample.decoderTimestampUs, sample);
    decoder.decode(new EncodedVideoChunk({
      type: sample.isKeyFrame
        ? 'key'
        : 'delta',
      /*
       ...
       * this value.
      */
      timestamp: sample.decoderTimestampUs,
      duration: timeToWebCodecsMicroseconds(sample.duration),
      data: sample.data,
    }));
  }
  await withTimeout(decoder.flush(), 10000, 'Exact-frame VideoDecoder.flush()');
  if (decoderFailure) {
    decoder.close();
  ...
  */
  const bitmap = await withTimeout(bitmapPromise, 10000, `Exact review sample ${request.targetSampleIndex}`);
  decoder.close();
  /*
...
async function decodeFile(file: File): Promise<void> {
  ...
  if (typeof VideoDecoder === 'undefined' || typeof EncodedVideoChunk === 'undefined') {
    throw new Error('This browser does not provide WebCodecs VideoDecoder. Use a current Chrome, Edge, Firefox, or Safari build.');
  }
  const mp4File = (MP4Box as any).createFile(true);
  debugDecoder('[Barnes decoder] discardMdatData:', mp4File.discardMdatData);
  let videoTrack: Mp4VideoTrack | null = null;
  ...
  let backgroundSampleIndices = new Set<number>();
  const backgroundSamples: BackgroundSample[] = [];
  const backgroundSamplePromises: Promise<void>[] = [];
  let demuxReadyResolve!: (track: Mp4VideoTrack) => void;
  let demuxReadyReject!: (reason: unknown) => void;
  const demuxReady = new Promise<Mp4VideoTrack>((resolve, reject) => {
  ...
  const decoderOutput = (frame: VideoFrame) => {
    try {
      ...
      if (queue && queue.length === 0)
        pendingTimingByTimestamp.delete(frame.timestamp);
      if (!timing) {
      ...
      if (backgroundSampleIndices.has(decodedIndex)) {
        const sampleFrame = frame.clone();
        const samplePromise = copyGrayscale(sampleFrame)
          .then((grayscale) => {
        ...
        backgroundSamplePromises.push(samplePromise);
      }
      if (representativeIndices.has(decodedIndex)) {
        const previewFrame = frame.clone();
        const previewPromise = processRepresentativeFrame(previewFrame, timing, decodedIndex)
          .then((preview) => {
            post({
        ...
        representativePromises.push(previewPromise);
      }
      emitTiming(timing);
      decodedFrames += 1;
      ...
      if (decodedFrames < 5) {
        debugDecoder('[Barnes decoder] output frame', {
          timestampUs: frame.timestamp,
          queueSize: decoder?.decodeQueueSize ?? null,
        });
      }
      if (decodedFrames == 1 ||
        decodedFrames % 10 === 0) {
  ...
  mp4File.onReady = async (info: Mp4Info) => {
    try {
      ...
      const nominalFps = track.duration > 0 &&
        track.nb_samples > 1
        ? (track.timescale *
          (track.nb_samples - 1)) / track.duration
        : null;
      const lastIndex = Math.max(0, track.nb_samples - 1);
      representativeIndices =
        new Set(evenlySpacedIndices(track.nb_samples, 9));
      backgroundSampleIndices =
        new Set(evenlySpacedIndices(track.nb_samples, 21));
  ...
  mp4File.onSamples = (trackId: number, _user: unknown, samples: Mp4Sample[]) => {
    if (!videoTrack) {
      extractionDoneReject(new Error('MP4Box delivered samples before video track initialization.'));
      return;
    }
    ...
    for (const sample of samples) {
      ...
      const sampleNumbers = samplesByPts.get(sample.cts) ?? [];
      sampleNumbers.push(sample.number ?? -1);
      samplesByPts.set(sample.cts, sampleNumbers);
    }
    ...
    debugDecoder('[Barnes decoder] MP4Box samples', {
    ...
    const deliveredAllByCount = extractedSampleCount >= videoTrack.nb_samples;
    const deliveredFinalSample = last?.number !== undefined &&
      last.number + 1 >= videoTrack.nb_samples;
    if (!extractionComplete &&
      (deliveredAllByCount ||
        deliveredFinalSample)) {
      extractionComplete = true;
      debugDecoder('[Barnes decoder] all MP4 samples extracted', {
        extractedSampleCount,
        finalSampleNumber: last?.number,
        expectedSamples: videoTrack.nb_samples,
      });
      const sortedRawPts = [...rawSamplePts].sort((a, b) => a - b);
      const rawIntervals: number[] = [];
      for (let i = 1; i < sortedRawPts.length; i += 1) {
        rawIntervals.push(sortedRawPts[i] -
          sortedRawPts[i - 1]);
      }
      const duplicateRawPts = [
        ...samplesByPts.entries(),
      ]
        .filter(([, sampleNumbers]) => sampleNumbers.length > 1);
      debugDecoder('[Barnes decoder] raw MP4 timing summary', {
        samples: rawSamplePts.length,
        uniquePts: new Set(rawSamplePts).size,
        duplicatePts: rawSamplePts.length -
        ...
        firstPts: sortedRawPts.at(0),
        lastPts: sortedRawPts.at(-1),
        duplicateExamples: duplicateRawPts.slice(0, 20),
      });
      extractionDoneResolve();
    }
    sampleChain = sampleChain
      .then(async () => {
        ...
        if (!decoder || !videoTrack) {
          throw new Error('Received samples before decoder initialization.');
        }
        for (const sample of samples) {
          if (cancelled)
            return;
          ...
          const pts = asRational(sample.cts, sample.timescale);
          const dts = asRational(sample.dts, sample.timescale);
          const duration = asRational(sample.duration, sample.timescale);
          const timestampUs = timeToWebCodecsMicroseconds(pts);
          ...
          const timestampQueue = pendingTimingByTimestamp.get(timestampUs) ?? [];
          timestampQueue.push(timing);
          pendingTimingByTimestamp.set(timestampUs, timestampQueue);
          decoder.decode(new EncodedVideoChunk({
            type: timing.isKeyFrame
              ? 'key'
              : 'delta',
            timestamp: timestampUs,
            duration: timeToWebCodecsMicroseconds(duration),
            data: sample.data,
          }));
          submittedSampleCount += 1;
          if (nextSampleIndex <= 5) {
            debugDecoder('[Barnes decoder] submitted chunk', {
        ...
        if (last?.number !== undefined) {
          mp4File.releaseUsedSamples(trackId, last.number + 1);
        }
      })
      .catch((error) => {
        extractionDoneReject(error);
  ...
  if (!activeDecoder)
    throw new Error('Decoder was not initialized.');
  debugDecoder('[Barnes decoder] starting flush', {
    decodedFrames,
    submittedSampleCount,
    decodeQueueSize: activeDecoder.decodeQueueSize,
    decoderState: activeDecoder.state,
  });
  await withTimeout(activeDecoder.flush(), 10000, 'VideoDecoder.flush()');
  await Promise.all(representativePromises);
  await Promise.all(backgroundSamplePromises);
  backgroundSamples.sort((a, b) => a.decodedIndex -
    b.decodedIndex);
  if (backgroundSamples.length === 0) {
    throw new Error('No frames were captured for background estimation.');
  }
  console.log('[Barnes tracking] background samples', {
    count: backgroundSamples.length,
    indices: backgroundSamples.map((sample) => sample.decodedIndex),
  });
  console.log("list", backgroundSamples.map((sample) => sample.decodedIndex));
  const backgroundModel = buildTemporalMedianBackground(backgroundSamples, readyTrack.video.width, readyTrack.video.height);
  post({
  ...
  debugDecoder('[Barnes decoder] flush complete', {
    decodedFrames,
    submittedSampleCount,
    decodeQueueSize: activeDecoder.decodeQueueSize,
  });
...
async function trackFile(
  request: Extract<DecoderRequest, {
    type: 'track-file';
  }>,
): Promise<void> {
  cancelled = false;
  const { file, background, calibration, settings, } = request;
  /*
  ...
  */
  if (background.width !==
    ...
    throw new Error('Arena calibration dimensions do not match the background model.');
  }
  const arenaMask = createArenaMask(calibration);
  const mp4File = (MP4Box as any).createFile(true);
  let videoTrack: Mp4VideoTrack | null = null;
  let decoder: VideoDecoder | null = null;
  let decodedFrames = 0;
  ...
  let nextSampleIndex = 0;
  let extractionComplete = false;
  const trackPoints: BodyTrackPoint[] = [];
  /*
  ...
  */
  const trackPointSampleIndex = new Map<BodyTrackPoint, number>();
  /*
   ...
   * FrameTiming.pts.
  */
  const pendingTimingByDecoderTimestamp = new Map<number, FrameTiming>();
  /*
  ...
  */
  let trackingChain = Promise.resolve();
  let trackingFailure: unknown = null;
  ...
  async function waitForTrackingCapacity() {
    while (!cancelled) {
      if (!decoder) {
        throw new Error('Tracking decoder was not initialized.');
      }
      if (decoder.state === 'closed') {
        throw new Error('Tracking decoder closed unexpectedly.');
      }
      if (decoder.decodeQueueSize <
  ...
  */
  const decoderOutput = (frame: VideoFrame) => {
    const timing = pendingTimingByDecoderTimestamp.get(frame.timestamp);
    if (timing) {
      pendingTimingByDecoderTimestamp.delete(frame.timestamp);
    }
    if (!timing) {
      frame.close();
      trackingFailure =
        new Error(`Tracked frame timestamp ${frame.timestamp} µs had no matching MP4 sample.`);
      return;
    }
    /*
  ...
  */
    const provisionalPresentationIndex = timing.sampleIndex;
    decodedFrames += 1;
      ...
      trackingChain
        .then(async () => {
          ...
          try {
            ...
            const point = trackBodyFrame(grayscale.pixels, timing, provisionalPresentationIndex, background, arenaMask, settings);
            trackPointSampleIndex.set(point, timing.sampleIndex);
            trackPoints.push(point);
            const completed = trackPoints.length;
            if (completed === 1 ||
              completed % 30 === 0) {
              post({
        ...
        .finally(() => {
          frame.close();
          pendingTrackingFrames =
            Math.max(0, pendingTrackingFrames -
              1);
        });
  };
  let demuxReadyResolve!: (track: Mp4VideoTrack) => void;
  ...
  mp4File.onReady =
    async (info: Mp4Info) => {
      try {
        const track = info.videoTracks?.[0];
        if (!track) {
          throw new Error('No video track found for tracking.');
        }
        if (!track.codec.startsWith('avc1') &&
          !track.codec.startsWith('avc3')) {
          throw new Error(`Tracking currently supports H.264/AVC MP4 only; found ${track.codec}.`);
        }
        if (track.video.width !==
          background.width ||
          track.video.height !==
          background.height) {
          throw new Error('Tracking video dimensions do not match the background model.');
        }
        videoTrack =
          track;
        const config: VideoDecoderConfig = {
  ...
  */
  mp4File.onSamples = (trackId: number, _user: unknown, samples: Mp4Sample[]) => {
    if (!videoTrack) {
      extractionDoneReject(new Error('Tracking samples arrived before track initialization.'));
      return;
    }
    extractedSampleCount +=
      samples.length;
    const last = samples.at(-1);
    const deliveredAllByCount = extractedSampleCount >=
      videoTrack.nb_samples;
    const deliveredFinalSample = last?.number !==
      undefined &&
      last.number + 1 >=
      ...
      sampleChain
        .then(async () => {
          ...
          for (const sample of samples) {
            ...
            */
            const decoderTimestampUs = sampleIndex;
            const timing: FrameTiming = {
              sampleIndex,
              presentationIndex: null,
              pts,
              ...
              */
              webCodecsTimestampUs: decoderTimestampUs,
              isKeyFrame: mp4SampleIsKey(sample),
            };
            pendingTimingByDecoderTimestamp.set(decoderTimestampUs, timing);
            decoder.decode(new EncodedVideoChunk({
              type: timing.isKeyFrame
                ? 'key'
                : 'delta',
              timestamp: decoderTimestampUs,
              duration: timeToWebCodecsMicroseconds(duration),
              data: sample.data,
            }));
            submittedSampleCount +=
  ...
  */
  await withTimeout(activeDecoder.flush(), 10000, 'Tracking VideoDecoder.flush()');
  /*
  ...
  */
  ...
  if (pendingTimingByDecoderTimestamp.size >
    0) {
    throw new Error(`${pendingTimingByDecoderTimestamp.size} tracking sample identities were not matched to decoded frames.`);
  }
  /*
   * Defensive ordering.
  */
 ...
 */
  trackPoints.sort((a, b) => {
    const timeDifference = (a.pts.ticks /
      a.pts.timescale) -
      (b.pts.ticks /
        b.pts.timescale);
    if (timeDifference !== 0) {
    ...
    return ((trackPointSampleIndex.get(a) ??
      Number.POSITIVE_INFINITY) -
      (trackPointSampleIndex.get(b) ??
        Number.POSITIVE_INFINITY));
  });
  for (let presentationIndex = 0; presentationIndex <
    trackPoints.length; presentationIndex += 1) {
    trackPoints[presentationIndex].presentationIndex =
      presentationIndex;
  }
  const detectedFrameCount = trackPoints.filter((point) => point.x !== null &&
    point.y !== null).length;
  const bodyTrack: BodyTrack = {
    width: readyTrack.video.width,
    height: readyTrack.video.height,
    points: trackPoints,
    detectedFrameCount,
    missingFrameCount: trackPoints.length -
      detectedFrameCount,
  };
  post({
    type: 'tracking-progress',
    completed: trackPoints.length,
    total: readyTrack.nb_samples,
  });
  post({
    type: 'track-complete',
    track: bodyTrack,
...
ctx.addEventListener('message', (event: MessageEvent<ExtendedDecoderRequest>) => {
  ...
  if (message.type ===
    'open-review-file') {
    openReviewFile(message.file).catch((error: unknown) => {
      post({
        type: 'review-error',
        requestId: null,
  ...
  if (message.type ===
    ...
    reviewDecodeChain =
      reviewDecodeChain
        .then(() => decodeReviewFrame(message))
        .catch((error: unknown) => {
          /*
  ...
  if (message.type ===
    'track-file') {
    trackFile(message).catch((error: unknown) => {
      post({
        type: 'error',
        message: 'Could not track mouse.',
        detail: error instanceof Error
          ? error.message
          : String(error),
      });
