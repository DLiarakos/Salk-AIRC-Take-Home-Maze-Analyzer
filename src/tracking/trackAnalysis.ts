...
export function detectionRateFromTrialStart(
  track: BodyTrack,
  startPresentationIndex: number,
): number {
  const trialPoints = track.points.filter(
    (point) => point.presentationIndex >= startPresentationIndex,
  );

  if (trialPoints.length === 0) return 0;

  const detected = trialPoints.filter(
