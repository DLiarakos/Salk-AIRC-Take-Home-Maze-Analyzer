...
export function processTrajectory(
  ...
  const prepared = prepareAnalysisTrajectory(
    track,
    trialWindow,
    settings.maxGapSeconds,
  );

  const medianFiltered = medianSmooth(
    prepared.points,
    settings.medianWindowSeconds,
  );

  const smoothed = meanSmooth(
    medianFiltered,
    settings.meanWindowSeconds,
  );
  ...
  return {
    ...
    qc: {
      ...
      segmentCount:
        new Set(prepared.points.map((point) => point.segmentId)).size,

      medianSmoothingCorrectionPixels: median(corrections),
      p95SmoothingCorrectionPixels: percentile(corrections, 0.95),
      maximumSmoothingCorrectionPixels:
        corrections.length > 0 ? Math.max(...corrections) : 0,
    },
