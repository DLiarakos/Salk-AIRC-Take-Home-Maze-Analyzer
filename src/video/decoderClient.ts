/**
 * Main-thread client API for worker-based video decoding and exact-frame review.
 *
 * Inputs:
 * - Browser File objects plus optional progress callbacks or requested review-frame indices.
 *
 * Outputs:
 * - DecodeResult metadata/timings/background/representative frames, cancellable decode tasks, and ExactFrameReviewSession objects.
 *
 * Main functions:
 * - decodeVideoFile() coordinates the full decode pass and validates returned frame timing.
 * - createExactFrameReviewSession() keeps a review worker open for frame-accurate interactive inspection.
 */
...
export function decodeVideoFile(
  file: File,
  onProgress?: (message: Extract<DecoderResponse, {
    type: 'progress';
  }>) => void,
): {
  promise: Promise<DecodeResult>;
  cancel: () => void;
} {
    ...
    worker.addEventListener('message', (event: MessageEvent<DecoderResponse>) => {
      ...
      switch (message.type) {
        ...
        case 'representative-frame':
          representativeFrames.push(message.frame);
          break;
        ...
        case 'done':
          ...
          if (!background) {
            reject(new Error('Decoder completed without a background model.'));
            worker.terminate();
            break;
          }
          {
            ...
            frames.sort((a, b) => a.pts.ticks / a.pts.timescale - b.pts.ticks / b.pts.timescale ||
              a.sampleIndex - b.sampleIndex);
            const presentationFrames = frames.map((frame, presentationIndex) => ({
              ...frame,
              presentationIndex,
            }));
            const timingValidation = validateFrameTimings(presentationFrames, metadata.frameCount, message.decodedFrames, metadata.trackTimescale);
            resolve({
...
type ReviewWorkerResponse = ReviewFileReadyResponse | ReviewFrameResponse | ReviewErrorResponse;
export interface ExactFrameReviewSession {
  ready: Promise<ExactFrameReviewReady>;
  requestFrame: (frame: FrameTiming) => Promise<ExactReviewFrame>;
  close: () => void;
}

export function createExactFrameReviewSession(file: File): ExactFrameReviewSession {
  const worker = new Worker(new URL('../workers/videoDecoder.worker.ts', import.meta.url), {
    type: 'module',
    name: 'barnes-exact-frame-review',
  });
  let closed = false;
  let nextRequestId = 1;
  let readyResolve!: (value: ExactFrameReviewReady) => void;
  let readyReject!: (reason: unknown) => void;
  const ready = new Promise<ExactFrameReviewReady>((resolve, reject) => {
    readyResolve =
      resolve;
    readyReject =
      reject;
  ...
  const pending = new Map<number, {
    resolve: (frame: ExactReviewFrame) => void;
  ...
  worker.addEventListener('message', (event: MessageEvent<ReviewWorkerResponse | DecoderResponse>) => {
    const message = event.data;
    if (message.type ===
      'review-file-ready') {
      readyResolve({
  ...
  return {
    ready,
    async requestFrame(frame: FrameTiming) {
      if (closed) {
        throw new DOMException('Exact-frame review session is closed.', 'AbortError');
      }
      if (frame.presentationIndex ===
        null) {
        throw new Error('Cannot review a frame without a presentation index.');
      }
      await ready;
      /*
      ...
      */
      for (const [requestId, request,] of pending) {
        request.reject(new DOMException('Superseded by a newer exact-frame request.', 'AbortError'));
        pending.delete(requestId);
      }
      const requestId = nextRequestId++;
      return new Promise<ExactReviewFrame>((resolve, reject) => {
        ...
        worker.postMessage({
          ...
          requestId,
          targetPresentationIndex: frame
            .presentationIndex,
          targetSampleIndex: frame
            .sampleIndex,
        });
      });
    },
    close() {
      ...
      closed =
        true;
      rejectPending(new DOMException('Exact-frame review session closed.', 'AbortError'));
      worker.terminate();
    },
  };
}
