/**
 * Visualization of the tracked mouse trajectory within the detected trial window.
 *
 * Inputs:
 * - BackgroundModel, BodyTrack, TrialWindow, and ArenaCalibration.
 *
 * Outputs:
 * - Canvas trajectory overlay and trial/tracking summary information for visual QC.
 *
 * Main component:
 * - TrajectoryPreview filters the track to the trial, samples display points, and avoids drawing across missing/timing gaps.
 */
import { useEffect, useMemo, useRef } from 'react';
import type { ArenaCalibration, BackgroundModel, BodyTrack, BodyTrackPoint, TrialWindow } from '../models/tracking';
...
function sampleForDisplay(points: BodyTrackPoint[], intervalSeconds = 0.1): BodyTrackPoint[] {
  ...
  for (const point of points) {
    ...
    const epsilon = 1e-6;
    if (lastTime === null || time - lastTime >= intervalSeconds - epsilon) {
      sampled.push(point);
      lastTime = time;
    }
  }
...
export default function TrajectoryPreview({ background, track, trialWindow, calibration, }: Props) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const trialPoints = useMemo(() => {
    const endIndex = trialWindow.endPresentationIndex ??
      Number.POSITIVE_INFINITY;
    return track.points.filter((point) => point.presentationIndex >= trialWindow.startPresentationIndex &&
      point.presentationIndex <= endIndex);
  }, [track, trialWindow]);
  const validPoints = useMemo(() => trialPoints.filter((point) => point.x !== null && point.y !== null), [trialPoints]);
  const displayPoints = useMemo(() => sampleForDisplay(validPoints, 0.1), [validPoints]);
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas)
      return;
    drawGrayscale(canvas, background.pixels, background.width, background.height);
    const context = canvas.getContext('2d');
    if (!context)
      return;
    /*
  ...
  const startSeconds = trialWindow.startPts.ticks /
    trialWindow.startPts.timescale;
  const lastPoint = validPoints.at(-1);
  const displayedDuration = lastPoint
    ? Math.max(0, ptsSeconds(lastPoint) - startSeconds)
    : null;
  return (<section className="card" aria-labelledby="trajectory-heading">
    ...
    <canvas ref={canvasRef} className="calibration-canvas" aria-label="Mouse body trajectory across the Barnes maze" />
