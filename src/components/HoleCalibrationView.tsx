/**
 * Interactive calibration and review interface for Barnes maze hole geometry.
 *
 * Inputs:
 * - BackgroundModel, ArenaCalibration, current HoleGeometry, and a geometry-change callback.
 *
 * Outputs:
 * - Complete/adjusted HoleGeometry including target-hole selection and manual hole-position corrections.
 *
 * Main component:
 * - HoleCalibrationView collects four anchors, generates the full hole layout, and supports manual geometric review.
 */
import { useEffect, useRef, useState } from 'react';
import type { PointerEvent as ReactPointerEvent } from 'react';
import type { ArenaCalibration, BackgroundModel, HoleGeometry } from '../models/tracking';
...
function canvasCoordinates(event: ReactPointerEvent<HTMLCanvasElement>) {
...
function drawBackground(canvas: HTMLCanvasElement, background: BackgroundModel) {
...
export default function HoleCalibrationView({ background, arena, geometry, onGeometryChange, }: Props) {
  ...
  const [draggedHoleIndex, setDraggedHoleIndex] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    if (geometry) {
      for (const hole of geometry.holes) {
        ...
        context.beginPath();
        context.arc(hole.centerX, hole.centerY, hole.radiusPixels, 0, Math.PI * 2);
        context.stroke();
        if (hole.manuallyAdjusted) {
          ...
          */
          ...
          context.beginPath();
          context.moveTo(hole.automaticCenterX, hole.automaticCenterY);
          context.lineTo(hole.centerX, hole.centerY);
          context.stroke();
          context.setLineDash([]);
          /*
          ...
          */
          const markerSize = 3;
          context.beginPath();
          context.moveTo(hole.automaticCenterX - markerSize, hole.automaticCenterY);
          context.lineTo(hole.automaticCenterX + markerSize, hole.automaticCenterY);
          context.moveTo(hole.automaticCenterX, hole.automaticCenterY - markerSize);
          context.lineTo(hole.automaticCenterX, hole.automaticCenterY + markerSize);
          context.stroke();
          context.restore();
        }
        context.setLineDash([]);
        context.font = '14px sans-serif';
        context.fillText(hole.isTarget ? 'T' : String(hole.index), hole.centerX + hole.radiusPixels + 3, hole.centerY);
      }
    }
    context.restore();
  }, [background, geometry]);
  function handlePointerDown(event: ReactPointerEvent<HTMLCanvasElement>) {
    ...
    */
    if (anchors.length < 4) {
      ...
      if (nextAnchors.length === 4) {
        onGeometryChange(createHoleGeometry(nextAnchors, holeCount, holeRadiusPixels));
      }
      return;
    }
    /*
    ...
    */
    ...
    for (const hole of geometry.holes) {
      const distance = Math.hypot(point.x - hole.centerX, point.y - hole.centerY);
    ...
    */
    const selectionRadius = closestHole.radiusPixels + 8;
    if (closestDistance > selectionRadius)
      return;
  ...
  function handlePointerMove(event: ReactPointerEvent<HTMLCanvasElement>) {
    ...
    onGeometryChange({
      ...geometry,
      holes: geometry.holes.map((hole) => hole.index === draggedHoleIndex
        ? {
          ...hole,
          centerX: nextX,
          centerY: nextY,
  ...
  function restoreHole(index: number) {
    ...
    onGeometryChange({
      ...geometry,
      holes: geometry.holes.map((hole) => hole.index === index
        ? {
          ...hole,
  ...
  return (<section className="card">
    ...
    <div className="calibration-controls">
      <label>
        ...
        <input type="number" min="4" step="1" value={holeCount} onChange={(event) => {
      ...
      <label>
        ...
        <input
          ...
          onChange={(event) => updateHoleRadius(Number(event.target.value))}
        />
        <span>px</span>
      </label>
    </div>
    ...
    {geometry && (<>
      ...
      {geometry && (<p>
        ...
        {geometry.holes.filter((hole) => hole.manuallyAdjusted).length}
        {' / '}
        {geometry.holes.length}
      </p>)}
