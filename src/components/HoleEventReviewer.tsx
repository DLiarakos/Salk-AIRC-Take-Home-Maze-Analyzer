/**
 * Frame-accurate manual review interface for hole-investigation events and related tracking corrections.
 *
 * Inputs:
 * - Source video/frame timings, automatic body track, hole geometry, investigation results/settings, and saved review state.
 *
 * Outputs:
 * - Confirmed/rejected/edited event decisions, manual event additions, trial-start overrides, and track-point corrections.
 *
 * Main responsibilities:
 * - Opens exact decoded review frames around detected events.
 * - Supports event confirmation, rejection, timing/hole edits, and manual additions.
 * - Supports manual trial-start and body-centroid corrections while preserving the automatic source track.
 */
...
import type {
  ...
  HoleInvestigationSettings, TrialWindow, TrialStartOverride, ManualHoleEventAddition, ManualTrackPointCorrectionMap,
} from '../models/tracking';
import { createExactFrameReviewSession } from '../video/decoderClient';
import type { ExactFrameReviewSession, ExactReviewFrame } from '../video/decoderClient';

export type HoleEventReviewStatus = 'unreviewed' | 'confirmed' | 'rejected' | 'edited';
export interface HoleEventReviewDecision {
  eventKey: string;
  status: HoleEventReviewStatus;
  note: string;
  ...
  */
  manualHoleIndex?: number | null;
  manualStartPresentationIndex?: number | null;
  manualEndPresentationIndex?: number | null;
}

export type HoleEventReviewDecisionMap = Record<string, HoleEventReviewDecision>;
interface Props {
  file: File;
  frames: FrameTiming[];
  track: BodyTrack;
  ...
  onDecisionsChange: (value: HoleEventReviewDecisionMap) => void;
  manualAdditions: ManualHoleEventAddition[];
  onManualAdditionsChange: (additions: ManualHoleEventAddition[]) => void;
  trialWindow: TrialWindow;
  automaticTrialWindow: TrialWindow;
  trialStartOverride: TrialStartOverride | null;
  onTrialStartOverrideChange: (value: TrialStartOverride | null) => void;
  automaticTrack: BodyTrack;
  manualTrackPointCorrections: ManualTrackPointCorrectionMap;
  onManualTrackPointCorrectionsChange: (value: ManualTrackPointCorrectionMap) => void;
}

type IndexedFrame = FrameTiming & {
  presentationIndex: number;
};
const REVIEW_PADDING_SECONDS = 0.50;
const CHECKPOINT_OFFSET_SECONDS = 0.20;
function frameSeconds(frame: FrameTiming): number {
  return (frame.pts.ticks /
    frame.pts.timescale);
}

function isIndexedFrame(frame: FrameTiming): frame is IndexedFrame {
  return (typeof frame.presentationIndex ===
    'number');
}

export function holeEventReviewKey(event: HoleInvestigationEvent): string {
  return [
    event.holeIndex,
...
function exactPtsKey(frame: FrameTiming): string {
  return (`${frame.pts.ticks}/` +
    `${frame.pts.timescale}`);
}

function nearestFrameByTime(frames: IndexedFrame[], targetSeconds: number): IndexedFrame | null {
  let best: IndexedFrame | null = null;
  let bestDistance = Number.POSITIVE_INFINITY;
  for (const frame of frames) {
    const distance = Math.abs(frameSeconds(frame) -
      targetSeconds);
...
export default function HoleEventReviewer(
  ...
  const trackCorrectionDraggingRef = useRef(false);
  const [selectedEventIndex, setSelectedEventIndex,] = useState(0);
  const [selectedPresentationIndex, setSelectedPresentationIndex,] = useState<number | null>(result.events[0]
  ...
  const [manualDraftHoleIndex, setManualDraftHoleIndex,] = useState<number>(geometry.holes[0]?.index ??
    0);
  const [manualDraftStartPresentationIndex, setManualDraftStartPresentationIndex,] = useState<number | null>(null);
  const [manualDraftEndPresentationIndex, setManualDraftEndPresentationIndex,] = useState<number | null>(null);
  const [manualDraftNote, setManualDraftNote,] = useState('');
  const [trackCorrectionXInput, setTrackCorrectionXInput,] = useState('');
  const [trackCorrectionYInput, setTrackCorrectionYInput,] = useState('');
  const [trackCorrectionNote, setTrackCorrectionNote,] = useState('');
  const [trackCorrectionClickMode, setTrackCorrectionClickMode,] = useState(false);
  /*
  ...
  */
  const indexedFrames = useMemo(() => frames.filter(isIndexedFrame), [frames]);
  const frameByIndex = useMemo(() => {
    const map = new Map<number, IndexedFrame>();
  ...
  useEffect(() => {
    ...
    session.ready
      ...
      .catch((error: unknown) => {
        ...
        setExactFrameError(error instanceof Error
          ? error.message
          : String(error));
      });
    return () => {
  ...
  const sourcePresentationIndices = useMemo(() => frames
    .map((frame) => frame.presentationIndex)
    .filter((presentationIndex): presentationIndex is number => typeof presentationIndex ===
      'number')
    .sort((a, b) => a - b), [
    frames,
  ]);
  const firstSourcePresentationIndex = sourcePresentationIndices[0] ??
    null;
  const lastSourcePresentationIndex = sourcePresentationIndices.at(-1) ??
    null;
  const pointByIndex = useMemo(() => {
    const map = new Map<number, BodyTrackPoint>();
    for (const point of track.points) {
      map.set(point.presentationIndex, point);
    }
    return map;
  }, [track]);
  const automaticPointByIndex = useMemo(() => {
    const map = new Map<number, BodyTrackPoint>();
    for (const point of automaticTrack.points) {
      map.set(point.presentationIndex, point);
    }
    return map;
  }, [
  ...
  const evidenceByIndex = useMemo(() => {
    const map = new Map<number, HoleInvestigationEvidence>();
    for (const observation of result.evidence) {
      map.set(observation.presentationIndex, observation);
    }
    return map;
  }, [result.evidence]);
  /*
  ...
  */
  const ptsCounts = useMemo(() => {
    const map = new Map<string, number>();
    for (const frame of indexedFrames) {
      const key = exactPtsKey(frame);
      map.set(key, (map.get(key) ?? 0) + 1);
    }
    return map;
  }, [indexedFrames]);
  const hasAutomaticEvents = result.events.length > 0;
  const activeEvent = hasAutomaticEvents
    ? (result.events[Math.min(selectedEventIndex, result.events.length - 1)] ?? null)
    : null;
  /*
  ...
  */
  useEffect(() => {
    if (result.events.length === 0) {
      ...
      setSelectedPresentationIndex(trialWindow
        .startPresentationIndex);
      return;
    }
    setSelectedEventIndex((current) => Math.min(current, result.events.length - 1));
  }, [
  ...
  */
  useEffect(() => {
    if (!activeEvent)
      return;
    setSelectedPresentationIndex(activeEvent
      .startPresentationIndex);
  }, [
  ...
  */
  const reviewFrames = useMemo(() => {
    if (!activeEvent)
      return [];
    const start = activeEvent.startTimeSeconds -
      REVIEW_PADDING_SECONDS;
    const end = activeEvent.endTimeSeconds +
      REVIEW_PADDING_SECONDS;
  ...
  */
  const closestEvidence = useMemo(() => {
    if (!activeEvent) {
      return null;
    }
    let best: HoleInvestigationEvidence | null = null;
    for (const observation of result.evidence) {
      if (observation.holeIndex !==
  ...
  const selectedFrame = selectedPresentationIndex === null
    ? null
    : (frameByIndex.get(selectedPresentationIndex) ?? null);
  useEffect(() => {
    if (!selectedFrame) {
      exactRequestGenerationRef
        .current += 1;
    ...
    const generation = ++exactRequestGenerationRef
      .current;
    /*
     * Clear the old image immediately so
     * metadata can never refer to frame B
     * while frame A remains visible.
    */
    exactBitmapRef
      .current
      ?.close();
    ...
    const timer = window.setTimeout(() => {
      ...
      /*
       * Avoid decoding every transient
       * slider position while dragging.
      */
      60);
    return () => {
  ...
  const selectedPoint = selectedPresentationIndex === null
    ? null
    : (pointByIndex.get(selectedPresentationIndex) ?? null);
  const selectedAutomaticPoint = selectedPresentationIndex === null
    ? null
    : (automaticPointByIndex.get(selectedPresentationIndex) ?? null);
  const selectedTrackCorrection = selectedPresentationIndex === null
    ? undefined
    : manualTrackPointCorrections[String(selectedPresentationIndex)];
  const selectedEvidence = selectedPresentationIndex === null
    ? null
    : (evidenceByIndex.get(selectedPresentationIndex) ?? null);
  useEffect(() => {
    ...
    const correction = manualTrackPointCorrections[String(selectedPresentationIndex)];
    if (correction?.kind ===
      'position') {
      setTrackCorrectionXInput(correction.x.toFixed(2));
      setTrackCorrectionYInput(correction.y.toFixed(2));
    }
    else {
      setTrackCorrectionXInput(selectedAutomaticPoint
        ?.x !== null &&
        ...
        ? selectedAutomaticPoint
          ...
          .toFixed(2)
        : '');
      setTrackCorrectionYInput(selectedAutomaticPoint
        ?.y !== null &&
        selectedAutomaticPoint
          ?.y !== undefined
        ? selectedAutomaticPoint
    ...
    setTrackCorrectionNote(correction?.note ??
      '');
  }, [
  ...
  const activeHole = activeEvent
    ? (geometry.holes.find((hole) => hole.index ===
      activeEvent.holeIndex) ?? null)
    : null;
  const currentReviewFrameIndex = useMemo(() => {
    if (selectedPresentationIndex ===
      null) {
      return -1;
    }
    return reviewFrames.findIndex((frame) => frame.presentationIndex ===
      selectedPresentationIndex);
  }, [
    reviewFrames,
    selectedPresentationIndex,
  ...
  const checkpoints = useMemo(() => {
    ...
    return {
      pre: nearestFrameByTime(reviewFrames, activeEvent
        .startTimeSeconds -
        CHECKPOINT_OFFSET_SECONDS),
      onset: frameByIndex.get(activeEvent
        .startPresentationIndex) ?? null,
      closest: closestEvidence
        ? (frameByIndex.get(closestEvidence
          .presentationIndex) ?? null)
        : null,
      end: frameByIndex.get(activeEvent
        .endPresentationIndex) ?? null,
      post: nearestFrameByTime(reviewFrames, activeEvent
        .endTimeSeconds +
        CHECKPOINT_OFFSET_SECONDS),
    };
  }, [
  ...
  */
  const trackCorrectionDraftPoint = trackCorrectionXInput.trim() !==
    '' &&
    trackCorrectionYInput.trim() !==
    '' &&
    Number.isFinite(Number(trackCorrectionXInput)) &&
    Number.isFinite(Number(trackCorrectionYInput))
    ? {
      x: Number(trackCorrectionXInput),
      y: Number(trackCorrectionYInput),
    }
    : null;
  /*
  ...
  */
  useEffect(() => {
    ...
    if (activeHole) {
      const entryRadius = activeHole.radiusPixels +
        settings.entryMarginPixels;
      const sustainRadius = activeHole.radiusPixels +
      ...
      */
      ...
      context.beginPath();
      context.arc(activeHole.centerX, activeHole.centerY, activeHole.radiusPixels, 0, Math.PI * 2);
      context.stroke();
      /*
       * Inner trigger ROI.
      */
      ...
      context.setLineDash([]);
      context.beginPath();
      context.arc(activeHole.centerX, activeHole.centerY, entryRadius, 0, Math.PI * 2);
      context.stroke();
      /*
       * Outer sustain ROI.
      */
      ...
      context.beginPath();
      context.arc(activeHole.centerX, activeHole.centerY, sustainRadius, 0, Math.PI * 2);
      context.stroke();
      context.restore();
    }
    if (selectedPoint?.x !== null &&
      ...
      */
      context.fillStyle =
        'white';
      context.beginPath();
      context.arc(selectedPoint.x, selectedPoint.y, 4, 0, Math.PI * 2);
      context.fill();
      /*
       * Centroid → estimated nose.
      */
      if (selectedPoint.noseX !== null &&
        ...
        context.lineTo(selectedPoint.noseX, selectedPoint.noseY);
        context.stroke();
        context.fillStyle =
          'magenta';
        context.beginPath();
        context.arc(selectedPoint.noseX, selectedPoint.noseY, 5, 0, Math.PI * 2);
        context.fill();
      }
    }
    if (trackCorrectionClickMode &&
      trackCorrectionDraftPoint) {
      /*
       * Draft manual centroid.
       * Crosshair + outer ring distinguishes
       * the unsaved manual position from the
       * effective saved centroid.
      */
      context.save();
      context.lineWidth = 2;
      context.strokeStyle =
        'yellow';
      context.beginPath();
      context.arc(trackCorrectionDraftPoint.x, trackCorrectionDraftPoint.y, 8, 0, Math.PI * 2);
      context.stroke();
      context.beginPath();
      context.moveTo(trackCorrectionDraftPoint.x -
        12, trackCorrectionDraftPoint.y);
      context.lineTo(trackCorrectionDraftPoint.x +
        12, trackCorrectionDraftPoint.y);
      context.moveTo(trackCorrectionDraftPoint.x, trackCorrectionDraftPoint.y -
        12);
      context.lineTo(trackCorrectionDraftPoint.x, trackCorrectionDraftPoint.y +
        12);
      context.stroke();
      context.restore();
    }
  ...
  const activeKey = activeEvent
    ? holeEventReviewKey(activeEvent)
    : null;
  const activeDecision = activeKey
    ? decisions[activeKey]
    : undefined;
  const activeStatus = activeDecision?.status ??
    'unreviewed';
  const activeNote = activeDecision?.note ?? '';
  const effectiveHoleIndex = activeEvent
    ? (activeStatus === 'edited'
      ? (activeDecision
        ?.manualHoleIndex ??
        activeEvent.holeIndex)
    ...
    : (geometry.holes[0]?.index ??
      0);
  const effectiveStartPresentationIndex = activeEvent
    ? (activeStatus === 'edited'
      ? (activeDecision
    ...
    : trialWindow
      .startPresentationIndex;
  const effectiveEndPresentationIndex = activeEvent
    ? (activeStatus === 'edited'
      ? (activeDecision
    ...
    : (trialWindow
      ...
      trialWindow
        .startPresentationIndex);
  const effectiveStartFrame = frameByIndex.get(effectiveStartPresentationIndex) ?? null;
  const effectiveEndFrame = frameByIndex.get(effectiveEndPresentationIndex) ?? null;
  function setDecision(status: Exclude<HoleEventReviewStatus, 'edited'>) {
    if (!activeEvent ||
      !activeKey) {
      return;
    }
    onDecisionsChange({
      ...
      [activeKey]: {
        ...
        */
        manualHoleIndex: null,
        manualStartPresentationIndex: null,
        manualEndPresentationIndex: null,
      },
    });
  }
  function applyEventEdit(
    patch: {
      manualHoleIndex?: number;
      manualStartPresentationIndex?: number;
      manualEndPresentationIndex?: number;
    },
  ) {
    if (!activeEvent ||
      !activeKey) {
    ...
    const currentHole = activeStatus === 'edited'
      ? (activeDecision
        ?.manualHoleIndex ??
        activeEvent.holeIndex)
      : activeEvent.holeIndex;
    const currentStart = activeStatus === 'edited'
      ? (activeDecision
        ?.manualStartPresentationIndex ??
        activeEvent
          .startPresentationIndex)
      : activeEvent
        .startPresentationIndex;
    const currentEnd = activeStatus === 'edited'
      ? (activeDecision
        ?.manualEndPresentationIndex ??
        activeEvent
          .endPresentationIndex)
      : activeEvent
        .endPresentationIndex;
    const nextHole = patch.manualHoleIndex ??
      currentHole;
    const nextStart = patch.manualStartPresentationIndex ??
      currentStart;
    const nextEnd = patch.manualEndPresentationIndex ??
      currentEnd;
    /*
    ...
    */
    ...
    onDecisionsChange({
      ...
      [activeKey]: {
        ...
        status: 'edited',
        note: activeNote,
        reviewedAtIso: new Date()
          .toISOString(),
        manualHoleIndex: nextHole,
        manualStartPresentationIndex: nextStart,
        manualEndPresentationIndex: nextEnd,
      },
    });
  }
  function setNote(note: string) {
    ...
    onDecisionsChange({
      ...
      [activeKey]: {
        ...
        status: activeStatus,
        note,
        reviewedAtIso: activeDecision
          ?.reviewedAtIso ??
          null,
        manualHoleIndex: activeDecision
          ?.manualHoleIndex ??
          null,
        manualStartPresentationIndex: activeDecision
          ?.manualStartPresentationIndex ??
          null,
        manualEndPresentationIndex: activeDecision
          ?.manualEndPresentationIndex ??
          null,
      },
    });
  }
  function selectFrame(frame: IndexedFrame | null) {
    if (!frame)
      return;
    setSelectedPresentationIndex(frame.presentationIndex);
  }
  function selectReviewFrame(index: number) {
    if (index < 0 ||
      index >= reviewFrames.length) {
      return;
    }
    selectFrame(reviewFrames[index]);
  }
  let confirmedCount = 0;
  let rejectedCount = 0;
  let editedCount = 0;
  for (const event of result.events) {
    const status = decisions[holeEventReviewKey(event)]?.status;
    if (status === 'confirmed') {
      confirmedCount += 1;
    }
    if (status === 'rejected') {
  ...
  function addManualInvestigation() {
    if (manualDraftStartPresentationIndex ===
      null ||
    ...
    const startFrame = frameByIndex.get(manualDraftStartPresentationIndex);
    const endFrame = frameByIndex.get(manualDraftEndPresentationIndex);
    if (!startFrame ||
      !endFrame) {
      return;
    }
    ...
    const nextOrdinal = manualAdditions.reduce((maximum, addition) => Math.max(maximum, addition.ordinal), 0) + 1;
    const now = new Date().toISOString();
    onManualAdditionsChange([
      ...manualAdditions,
      {
        id: makeManualAdditionId(),
        ordinal: nextOrdinal,
        holeIndex: manualDraftHoleIndex,
        startPresentationIndex: manualDraftStartPresentationIndex,
        endPresentationIndex: manualDraftEndPresentationIndex,
        note: manualDraftNote,
        createdAtIso: now,
        updatedAtIso: now,
      },
    ]);
  ...
  const manualDraftOutsideTrial = manualDraftStartPresentationIndex !==
    null &&
    manualDraftEndPresentationIndex !==
    null &&
    (manualDraftStartPresentationIndex <
      ...
      (trialWindow
        ...
        trialWindow
          .endPresentationIndex));
  const parsedTrackCorrectionX = Number(trackCorrectionXInput);
  const parsedTrackCorrectionY = Number(trackCorrectionYInput);
  const trackCorrectionPositionValid = selectedPresentationIndex !==
    null &&
    trackCorrectionXInput.trim() !==
    '' &&
    trackCorrectionYInput.trim() !==
    '' &&
    Number.isFinite(parsedTrackCorrectionX) &&
    Number.isFinite(parsedTrackCorrectionY) &&
    parsedTrackCorrectionX >= 0 &&
    parsedTrackCorrectionX <
    track.width &&
    parsedTrackCorrectionY >= 0 &&
    parsedTrackCorrectionY <
    track.height;
  function setTrackCorrectionDraftFromPointer(event: ReactPointerEvent<HTMLCanvasElement>) {
    if (selectedPresentationIndex ===
      null) {
      return;
    }
    ...
    const rawY = (event.clientY -
      ...
      (track.height /
        rect.height);
    const x = Math.max(0, Math.min(track.width - 1, rawX));
    const y = Math.max(0, Math.min(track.height - 1, rawY));
    setTrackCorrectionXInput(x.toFixed(2));
    setTrackCorrectionYInput(y.toFixed(2));
  }
  function saveManualTrackPosition() {
    if (!trackCorrectionPositionValid ||
      selectedPresentationIndex ===
      null) {
      return;
    }
    const key = String(selectedPresentationIndex);
    onManualTrackPointCorrectionsChange({
      ...manualTrackPointCorrections,
      [key]: {
        presentationIndex: selectedPresentationIndex,
        kind: 'position',
        x: parsedTrackCorrectionX,
        y: parsedTrackCorrectionY,
        note: trackCorrectionNote,
        updatedAtIso: new Date()
          .toISOString(),
      },
    });
    setTrackCorrectionClickMode(false);
  }
  function markCurrentTrackPointMissing() {
    if (selectedPresentationIndex ===
      null) {
      return;
    }
  ...
  function resetCurrentTrackPoint() {
    if (selectedPresentationIndex ===
      null) {
      return;
    }
    ...
    delete next[key];
    onManualTrackPointCorrectionsChange(next);
    setTrackCorrectionClickMode(false);
  }
  function removeManualInvestigation(id: string) {
    onManualAdditionsChange(manualAdditions.filter((addition) => addition.id !== id));
  }
