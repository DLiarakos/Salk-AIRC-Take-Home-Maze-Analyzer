...
export function estimateBodyShape(
  ...
  const majorEigenvalue =
    (trace + difference) / 2;

  const minorEigenvalue =
    (trace - difference) / 2;
   ...
   */
  const low =
    quantile(projections,0.08);

  const high =
    quantile(projections,0.92);
...
export function resolveTrackOrientation(
  track: BodyTrack,
  ...
  const points =
    track.points.map((point) => {
      ...
      let useA: boolean | null =
        null;

      let confidence = 0;

      let method:
        BodyTrackPoint['orientationMethod'] =
          'unresolved';
...
export function summarizeOrientation(
  ...
  return {
    ...
    motionResolvedPoints:
      motion.length,

    continuityResolvedPoints:
      continuity.length,
