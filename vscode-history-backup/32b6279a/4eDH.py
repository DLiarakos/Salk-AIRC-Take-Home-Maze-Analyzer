...
# revert name of mel_ucla_2016 study to be compatible with pre-curated clinical folder
if os.path.exists('mel_ucla_2016'):
	os.rename('mel_ucla_2016','skcm_ucla_2016')


# if data_CNA file is absent from the study, remove it.
for idir in os.listdir(cwd_raw_data):
    if os.path.isdir(os.path.join(cwd_raw_data,idir)):
        flist=os.listdir(os.path.join(cwd_raw_data,idir))
        flist=[ifile for ifile in flist if os.path.isfile(os.path.join(cwd_raw_data,idir,ifile))]
        if 'data_CNA.txt' not in flist:
            shutil.rmtree(os.path.join(cwd_raw_data,idir))


# Process The folder to remove any  files unnecessary for CNA analysis
remlist=[]
for idir in os.listdir():
    if os.path.isdir(idir):
        for fname in os.listdir(idir):
            if os.path.isfile(os.path.join(idir,fname)):
                if ('clinical' not in fname.lower()) and  ('cna' not in fname.lower()) and ('license' not in fname.lower()) :
                    rempath=os.path.join(idir,fname)
                    remlist=remlist+[rempath]
                    os.remove(rempath)
# Store the names of removed files in a log 
with open('Removed_files.txt','w') as fremove:
    for element in remlist:
        print(element,file=fremove)