...
for (const observation of evidence) {
   ...
   * NO ACTIVE EVENT
   *
   * Merely entering the outer sustain zone does not
   * start an investigation. The nose must cross the
   * smaller entry radius.
   */
  if (candidate === null) {
    if (
      isPositive &&
   ...
   * inside the outer sustain radius keep the event
   * alive. They no longer need to remain in the
   * smaller entry radius.
   */
  if (
    isPositive &&
    ...
    if (
      ...
      /*
       * This observation may immediately begin a new
       * event only if it is itself inside the inner
       * trigger radius.
       */
   ...
   *
   * A different hole can immediately take over only
   * when its INNER entry radius has been reached.
   */
  if (
    isPositive &&
    observation.holeIndex !==
  ...
  /*
   * Outside, unknown, misaligned, or only in another
   * hole's sustain region:
   *
   * treat as an interruption. Brief interruptions may
   * still be bridged by the existing visible setting.
   */
  const interruption =
    observation.timeSeconds -
    candidate.lastPositive.timeSeconds;
...
/*
 * Flush any investigation still active at trial end.
 */
finishCandidate(candidate);
candidate = null;
}
