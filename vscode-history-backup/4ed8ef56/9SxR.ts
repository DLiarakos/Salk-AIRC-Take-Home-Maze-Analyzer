...
const uniqueIncorrectHoleCount =
  uniqueIncorrectHoleIndices.length;

const repeatedPrimaryErrorCount =
  Math.max(
    0,
    primaryErrorCount -
 ...
 * hole 10 → distance 10
 */
const maximumIncorrectHoleDistanceFromTarget =
  uniqueIncorrectHoleIndices.length >
  0
    ? Math.max(
        ...uniqueIncorrectHoleIndices.map(
          (holeIndex) =>
...
reasoning.push(
  `${primaryErrorCount} primary error event(s) occurred across ${uniqueIncorrectHoleCount} unique incorrect hole(s).`,
);

if (
  repeatedPrimaryErrorCount > 0
) {
  reasoning.push(
    `${repeatedPrimaryErrorCount} error event(s) were repeated investigations of previously visited incorrect holes.`,
  );
}
...
if (
  ...
  reasoning.push(
    `The farthest incorrect hole was ${maximumIncorrectHoleDistanceFromTarget} hole-step(s) from the target.`,
  );
}
  } else if (
    serialQualified
