...
export function detectEscapeCandidate(
  ...
  if (
    ...
    const baselineBodyArea =
      bodyAreas.length % 2 === 1
        ? bodyAreas[middle]
        : (
            bodyAreas[middle - 1] +
            bodyAreas[middle]
    ...
    const terminalStartTime =
      recordingEndTime -
      settings.escapeLookbackSeconds;

    const terminalPoints =
      detectedPoints.filter(
        (point) =>
          pointSeconds(point) >=
          terminalStartTime,
      );
    ...
    if (
      ...
      if (
        ...
        const collapsedMiddle =
          Math.floor(
            collapsedAreas.length / 2,
          );

        const terminalBodyArea =
          collapsedAreas.length % 2 === 1
            ? collapsedAreas[
                collapsedMiddle
              ]
            : (
        ...
        const terminalBodyAreaFraction =
          baselineBodyArea > 0
            ? (
                terminalBodyArea /
                baselineBodyArea
