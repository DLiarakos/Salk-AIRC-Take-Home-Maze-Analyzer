...
const exactRequestGenerationRef =
  useRef(0);
const trackCorrectionDraggingRef =
  useRef(false);
  const [
    selectedEventIndex,
...
const trackCorrectionDraftPoint =
...
if (bitmap) {
  ...
  context.beginPath();
  context.arc(
    trackCorrectionDraftPoint.x,
    trackCorrectionDraftPoint.y,
    8,
    0,
    Math.PI * 2,
  );
...
const parsedTrackCorrectionX =
  Number(
    trackCorrectionXInput,
  );

const parsedTrackCorrectionY =
  Number(
    trackCorrectionYInput,
  );

const trackCorrectionPositionValid =
  selectedPresentationIndex !==
    null &&
  trackCorrectionXInput.trim() !==
    '' &&
...
function setTrackCorrectionDraftFromPointer(
  event:
...
<canvas
  ...
  onPointerDown={(event) => {
    ...
    setTrackCorrectionDraftFromPointer(
      event,
    );
  }}
