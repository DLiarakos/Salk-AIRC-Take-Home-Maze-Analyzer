...
async function copyNativeLuma(
  ...
  if (
    ...
    throw new Error(
      `Native frame format ${format ?? 'unknown'} ` +
  ...
  /*
   * For both I420 and NV12, plane 0 is the
   * full-resolution luminance (Y) plane.
   */
  const yPlane = layouts[0];

  if (!yPlane) {
    throw new Error(
      `${format} frame did not provide a Y plane.`,
    );
  }
  ...
  /*
   * Do not assume row stride == image width.
   * Hardware decoders may pad rows.
   */
  for (
    let row = 0;
    row < height;
...
export async function copyGrayscale(
  ...
  if (
    ...
    return copyNativeLuma(frame);
  }

  /*
   * Safe compatibility path for packed RGB,
   * unusual decoder formats, or browsers where
   * direct luma access is unavailable.
   */
